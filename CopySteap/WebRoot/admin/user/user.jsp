...
function del_tr(id){
    ...
    var userIds = tr.cells[1].innerHTML+",";	//得到要删除的用户的编号
...
function alt_tr(id) {
	...
	function alterNoOfTable() {
		...
		for (var i = 1; i < len; i++) {
			$('#userTable tr:eq(' + i + ') td:first').html(
	...
	function showRoleTable(result) {
		...
		for (var i = 0; i < roleList.length; i++) {
			...
			$("#userRoleTable").append(
					"<tr id='role_tr_"+index+"'> <td>" + index
							+ "</td><td class='hide'>" + roleList[i].roleId
							+ "</td><td>" + roleList[i].roleName + "</td><td>"
